...
# Remplir la table tracks avec les musiques des playlists
...
def save_tracks(tracks, emotion, genre, playlist_id):
    ...
    for track in tracks:
        try:
            # INSERT OR IGNORE evite les doublons sur spotify_id unique
            cursor.execute(
                """
...
def main():
    ...
    for i, playlist in enumerate(PLAYLISTS[START_INDEX:], start=START_INDEX):
        print(
            f"[{i+1}/{len(PLAYLISTS)}] Import playlist {playlist['playlist_id']} | {playlist['emotion']} | {playlist['genre']}"
        )

        # Appel Spotify + normalisation des infos track
        tracks = fetch_playlist_tracks(playlist["playlist_id"])
        print("Nombre de tracks recuperees :", len(tracks))
        ...
        # Pause pour éviter le rate limit et se faire bloquer
